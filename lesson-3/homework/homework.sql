...
SELECT 
    ROW_NUMBER() OVER (ORDER BY EmpID) + 10,
    CASE WHEN Salary > 6000 THEN 'Management' ELSE 'General' END,
    EmpID
FROM Employees
WHERE EmpID IN (SELECT TOP 5 EmpID FROM Employees ORDER BY EmpID);
...
WHERE NOT EXISTS (
    SELECT 1 
    FROM SalaryHistory
    WHERE SalaryHistory.EmpID = Employees.EmpID
    AND SalaryHistory.ChangeDate >= DATEADD(YEAR, -2, GETDATE())
);
...
SELECT * INTO #Employees_Updates FROM Employees WHERE 1=0;
INSERT INTO #Employees_Updates VALUES (1, 'Updated Name', 7500), (100, 'New Employee', 5000);
